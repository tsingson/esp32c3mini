...
// 💡 提示：测试阶段强烈建议将其改为 (5ULL * 1000ULL * 1000ULL) 也就是 5
// 秒，以便极速验证连续多轮循环
...
static void IRAM_ATTR gpio_isr_handler(void *arg) {
  ...
  xTaskNotifyFromISR(sleep_task_handle, 0, eNoAction,
...
static void save_sequence_and_enter_deep_sleep(const char *log_prefix) {
  ...
  ESP_LOGW(TAG, ">>> %s [NVS 写入成功] 最新持久化序号已变更为: %ld", log_prefix,
           boot_isr_seq);
  ESP_LOGW(TAG, ">>> 正在剥离引脚并锁闭总线，芯片即将进入深睡眠...");
  fflush(stdout);
  vTaskDelay(pdMS_TO_TICKS(
      200)); // 预留物理窗口期，确保内置 USB-CDC 芯片发完最后一批数据
...
static void worker_task(void *arg) {
  ...
  while (1) {
    ...
    ESP_LOGI("WORKER",
             "【工作正常】持久化序号 = %ld (当前运行进度: %ld / 30 秒)",
             boot_isr_seq, elapsed_ms / 1000);
...
void app_main(void) {
  ...
  xTaskCreate(nvs_and_sleep_task, "flash_sleep_task", 4096, NULL,
              configMAX_PRIORITIES - 1, &sleep_task_handle);
  ...
  gpio_isr_handler_add(BOOT_BUTTON_PIN, gpio_isr_handler,
                       (void *)BOOT_BUTTON_PIN);
}
